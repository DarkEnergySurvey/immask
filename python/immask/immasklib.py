...
def create_logger(level=logging.NOTSET, log_format=None, log_format_date=None):
    """
...
def uint2int(array):
    """ Utility function to convert from unsigned to signed integer.
    This function uses the fact that the unique character codes for
    signed integers are lower case of their signed equivalents.
    http://docs.scipy.org/doc/numpy/reference/arrays.dtypes.html
    """
    # Could add a few sanity checks here...
    char = array.dtype.char
    return array.view(char.lower())


def int2bit(value):
    """ Utility function to convert from integer to binary bit. """
    return int(np.log(value) / LOG2)
...
class BaseMasker:
    ...
    def run(self):
        ...
        raise Exception("You called run on the base class")

    def write(self):
        """ Write the masker QA files """
        raise Exception("You called write on the base class")
    ...
    @staticmethod
    def argparser(obj, title, **kwargs):
        """
        ...
        arparser = argparse.ArgumentParser(**kwargs)
        group = arparser.add_argument_group(title)
...
class CosmicMasker(BaseMasker):
    ...
    LSSTMaskPlaneDict = dict(
        ...
        SUSPECT=int2bit(MASKBITS.BADPIX_SUSPECT),  # These bit definitions are added
    ...
    def __init__(self, image, outdir, **kwargs):
        # instantiating these here rather than in other methods
        self.interpCR = None
        self.nGrowCR = None
        self.updateWeightCR = None
        self.fwhm = None
        self.minSigma = None
        self.min_DN = None
        self.fractionCR = None
        self.msk = None
        self.scia = None
        self.mska = None
        self.mi = None
        self.crs = None
        self.NCRs = None
        BaseMasker.__init__(self, image, outdir, **kwargs)
    ...
    def make_lsst_image(self):
        ...
        scidata = copy.copy(self.image.data)
        wgtdata = copy.copy(self.image.weight)

        # Could be done with DESImage.get_variance (but doesn't fix bad values)
        WGT_fixed = np.where(wgtdata <= 0, wgtdata.max()/1e6, wgtdata)  # Fix values < 0
        variance = 1/WGT_fixed

        # 1 - Create the science image
        afwsci = afwImage.ImageF(scidata)
        # 2 - Create the mask plane (must be unsigned int)
        self.msk = self.make_lsst_mask()
        # 3 - The variance image
        afwvar = afwImage.ImageF(variance)
        # Into numpy-arrays to handle some numpy fast operations
        self.scia = afwsci.getArray()
        self.mska = self.msk.getArray()

        # Make an LSST masked image (science, mask, and weight)
        self.mi = afwImage.makeMaskedImage(afwsci, self.msk, afwvar)

    def find_cosmics(self):
        """
    ...
    def fix_cosmics(self):
        ...
        This needs to be run after the CRs are detected (ie, after
        `find_cosmics`).
          IMAGE: If requested ('--interpCRs', the image plane will be
          interpolated
          MASK:  The mask plane will have the BADPIX_CRAY bit set.
          WEIGHT:If requested ('--updateWeightCR'), the weight plane will be
          set to zero.
        """
        ...
        # This controls when the interpolated values make it back to
        # the DESImage
        if self.interpCR:
    ...
    def update_header(self):
        ...
        """
        rec = dict(name='DESNCRAY', value=self.NCRs, comment="Number of masked cosmic rays")
...
class StreakMasker(BaseMasker):
    ...
    defaults = odict([
        ['bkgfile', dict(default=None, help="Input background FITS file (fz/fits)")],
        ['draw', dict(action='store_true', help="Draw diagnostic plots.")],
        ['draw_basename', dict(default=False, help="File basename diagnostic plots.")],
        ['template_dir', dict(default="/dev/null", help="Directory containing Hough template.")],
        ['galfile', dict(default=None, help="Galaxy masking file")],  # Image pre-processing
        ['bin_factor', dict(default=8, type=int, help="Binning factor to beat down sky noise")],  # For detection, merging, and characterization
        ...
        ['nsig_mask', dict(default=8., type=float,
                           help="Threshold for Hough peak characterization")],  # Quality cuts
        ['max_width', dict(default=150., type=float, help="Maximum streak width in (pix)")],
        ['max_angle', dict(default=15., type=float, help="Maximum streak angular extent (deg)")],
        ['min_fill', dict(default=0.25, type=float,
                          help="Minimum fraction of streak above threshold")],  # For clipping partial streaks and diffraction spikes
        ['clip', dict(action="store_true", help="Clip diffraction spikes (DEPRECATED).")],
        ...
        ['clip_range', dict(default=10., type=float,
                            help="Allowed clip angle range (deg); >90 to accept all angles")], # For masking
        ...
        ['maxmask', dict(default=1000, type=int,
                         help="Maximum number of streaks to mask [NOT IMPLEMENTED]")],  # Update Weight maps
        ['updateWeightStreaks', dict(default=False, action="store_true",
                                     help="Update the Weight Map for streaks.")],  # Streak objects (probably don't need both)
    ...
    def __init__(self, image, outdir, **kwargs):
        ...
        self.bkgfile = None
        self.draw = None
        self.draw_basename = None
        self.template_dir = None
        self.galfile = None
        self.bin_factor = None
        self.nsig_sky = None
        self.nsig_detect = None
        self.nsig_merge = None
        self.nsig_mask = None
        self.max_width = None
        self.max_angle = None
        self.min_fill = None
        self.clip = None
        self.nsig_clip = None
        self.clip_angle = None
        self.clip_range = None
        self.mask_factor = None
        self.maskbits = None
        self.setbit = None
        self.maxmask = None
        self.updateWeightStreaks = None
        self.write_streaks = None
        self.streaksfile = None
        self.template = None
    ...
    def run(self):
        ...
        if self.galfile:
            self.masked = self.masked | self.mask_galaxies(self.galfile)
        ...
        if maskfrac > 0.85:
            logging.warning("Image is too heavily masked; skipping...")
            self.subIm.fill(1)
        ...
        if self.detect_nlabel:
            hist, _, rev = self.idl_histogram(self.detect_label, bins=np.arange(self.detect_nlabel + 2))
        ...
        if merge_nlabel:
            hist, _, rev = StreakMasker.idl_histogram(merge_label, bins=np.arange(merge_nlabel + 2))
            good = []
        ...
        if mask_nlabel:
            hist, _, rev = StreakMasker.idl_histogram(self.mask_label, bins=np.arange(mask_nlabel+2))
            good = []
    ...
    def draw_plots(self):
        ...
        _, axes = plt.subplots(1, 3, dpi=dpi)
        sigma = [1, 2, 3]
        for ax, sig in zip(axes, sigma):
            ...
            ax.set_title(rf"Sky Noise (${sig}\sigma$)")
        ...
        ymax, xmax = self.searchIm.shape
        x = np.arange(xmax)
        _, axes = plt.subplots(1, 3, dpi=dpi)
        titles = ["Sky Noise", "Streaks", "Mask"]
        for ax, t in zip(axes, titles):
            ax.imshow(self.searchIm, origin='bottom')
            ax.set_title(t)
        plt.sca(axes[1])
        for obj in self.mask_objs:
            y1 = obj['SLOPE'] * x + obj['INTER1']
            plt.plot(x, y1, '--w')
            y2 = obj['SLOPE'] * x + obj['INTER2']
            plt.plot(x, y2, '--w')
        ...
        pngfile = f"{drawbase}_hough.png"
        logging.info("Drawing Hough transform: %s", pngfile)
        _, axes = plt.subplots(2, 2, figsize=(12, 6), sharey='all', sharex='all', dpi=dpi)
        axes = axes.flatten()
        titles = ["Hough", "Normalized", fr'Detect $({self.nsig_detect:g} \sigma)$',
                  fr"Mask $({self.nsig_mask:g} \sigma)$"]
        ...
        kw = [dict(kwargs), dict(kwargs, vmax=35), dict(kwargs), dict(kwargs), ]
    ...
    def get_template(self):
        ...
        if os.path.exists(filename):
            ...
            self.template = StreakMasker.read_template(filename)[0]
        else:
            ...
            corrections = odict([
                ('edge', [self.mask_edges, 256 / self.bin_factor, 1.0]), ('corner', [self.mask_corners, 256 / self.bin_factor, 5.0]), ('triangle', [self.mask_triangle_corners, 320 / self.bin_factor, 8.0]), ])
            ...
            self.template = self.template + weight * temp

    def update_header(self):
        """
        Add records to the header of the fits files
        """
        rec = dict(name='DESNSTRK', value=len(self.mask_objs), comment="Number of streaks masked")
        self.image.header.add_record(rec)
    ...
    def mask_galaxies(self, filename, bit=MASKBITS.BADPIX_STAR, factor=1.7):
        ...
        data = np.genfromtxt(filename, delimiter=',', names=names, dtype=None, encoding="us-ascii")
        ...
        delta_ra = size / np.cos(np.radians(dec))
        delta_dec = size
        ramin = ra - delta_ra
        ramax = ra + delta_ra
        decmin = dec - delta_dec
    ...
    def streak_statistics(array, rho, theta, good, rev):
        ...
        """
        ngood = len(good)
        objs = np.recarray((ngood, ), dtype=[('LABEL', 'i4'), ('NPIX', 'f4'), ('MAX', 'f4'), ('MAX_X0', 'i4'), ('MAX_Y0', 'i4'), ('XMIN', 'i4'), ('XMAX', 'i4'), ('YMIN', 'i4'), ('YMAX', 'i4'), ('CEN_X0', 'f4'), ('CEN_Y0', 'f4'), ('CEN_Y1', 'f4'), ('CEN_Y2', 'f4'), ('DELTA_THETA', 'f4'), ('DELTA_RHO', 'f4'), ('SLOPE', 'f4'), ('INTER1', 'f4'), ('INTER2', 'f4'), ('WIDTH', 'f4'), ('BINNING', 'i4'), ('FILL_FRAC', 'f4'), ('CORNERS', 'f4', (4, 2)), ('CORNERS_WCS', 'f8', (4, 2)), ('CUT', 'i2'), ])
        ...
        for i in range(0, ngood):
            ...
            tree = cKDTree(pix)
            _, idx = tree.query([centroid[1], centroid[0]])
            xcent, ycent = pix[idx]
            objs[i]['CEN_X0'] = xcent
            objs[i]['CEN_Y0'] = ycent
    ...
    def create_polygon(mask, pad=0.5):
        ...
        xmin_ymin = xmin_ypix.min()
        xmin_ymax = xmin_ypix.max()
        ...
        ymax_xmin = ymax_xpix.min()
        ymax_xmax = ymax_xpix.max()
    ...
    def mask_polygon(mask, vertices):
        ...
        nypix, nxpix = out_mask.shape[0:2]
    ...
    def mask_triangle_corners(mask, npix=32):
        """ Mask image corners with triangles """
        out_mask = np.zeros(mask.shape, dtype=mask.dtype)
        n, m = mask.shape
    ...
    def clip_mask(image, masked, mask, slope, nsig_clip=4):
        ...
        # Should always be <= 1
        density = StreakMasker.fill_fraction(image, masked, mask)
    ...
    def write_template(shape):
        ...
        data = np.recarray(1, dtype=[('template', 'u8', template.shape), ('theta', 'f8', theta.size), ('rho', 'f8', rho.size)])
    ...
    def mmm(sky_vector, mxiter=50, atol=1e-10):
        ...
        if nsky <= minsky:
            raise ValueError(f"Input vector must contain at least {minsky} elements")

        nlast = nsky - 1  # hmmm
        sky = np.sort(sky_vector)

        skymid = 0.5 * sky[int((nsky - 1) / 2)] + 0.5 * sky[int(nsky / 2)]  # median
        ...
        while redo:
            ...
            if not done:
                ...
                while not done:
                    newmin = newmin + istep
                    done = newmin in (-1, nlast)
                    if not done:
                        done = sky[newmin] <= cut1 <= sky[newmin+1]
                if tst_min:
                    delta = sky[newmin+1:minimm] - skymid
                else:
                    delta = sky[minimm+1:newmin] - skymid
                mysum = mysum - istep*delta.sum()
            ...
            if not done:
                ...
                while not done:
                    newmax = newmax + istep
                    done = newmax in (nlast, -1)
                    if not done:
                        done = sky[newmax] <= cut2 <= sky[newmax+1]
                if tst_max:
    ...
    def read_bkg_image(bkgfile):
        ...
        if not os.path.exists(bkgfile):
            raise IOError(f"File: {bkgfile} does not exists... bye")
        ...
        if image_ext is None:
            raise ValueError(f"Cannot find IMAGE extension via DES_EXT in {bkgfile}")
    ...
    def median_filter(data, factor=4):
        return StreakMasker.percentile_filter(data, factor, q=50)
...
def cmdline():
    ...
    cmdparser = argparse.ArgumentParser(description=description, formatter_class=formatter)
    ...
    cosmics = BaseMasker.argparser(CosmicMasker, title, add_help=False)
    _ = subparsers.add_parser(title, description=description, parents=[general, cosmics], formatter_class=formatter, help=description)
    ...
    streaks = BaseMasker.argparser(StreakMasker, title, add_help=False)
    _ = subparsers.add_parser(title, description=description, parents=[general, streaks], formatter_class=formatter, help=description)
    ...
    _ = subparsers.add_parser(title, description=description, parents=[general, cosmics, streaks], formatter_class=formatter, help=description)
    #subparser.set_defaults(func=run_all)
    cmdparser.set_defaults(func=run)
...
def run(inargs):
    ...
    if 'all' in commands:
        commands = ['cosmics', 'streaks']
    for command in commands:
        logging.info('Running %s...', command)
        start = time.time()
        if command == 'cosmics':
            CosmicMasker(image, **kwargs)
        if command == 'streaks':
            StreakMasker(image, **kwargs)
        #  In the future...
        # if command == 'stars':
    ...
    # otherwise will use the enviromental
    # variable $DESPYFITS_USE_INDIRECT_WRITE
    if inargs.indirect_write:
        despyfits.DESImage.use_indirect_write = inargs.indirect_write
    if inargs.indirect_write_prefix:
...
def elapsed_time(t1, verb=False):
    """ Format time output """
    t2 = time.time()
    stime = f"{int((t2 - t1) / 60.)}m {(t2 - t1) - 60 * int((t2 - t1) / 60.):2.2f}s"
    if verb:
